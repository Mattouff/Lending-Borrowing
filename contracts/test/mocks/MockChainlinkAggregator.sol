...
contract MockChainlinkAggregator is Ownable {
     ...
     */
    constructor(int256 price, uint8 decimalsValue, string memory descriptionText) Ownable(msg.sender) {
        _price = price;
    ...
    function setTimestampAgo(uint256 secondsAgo) external onlyOwner {
        // Ensure we never set updatedAt to 0 as that bypasses staleness checks
        if (secondsAgo >= block.timestamp) {
            // Set to a very old timestamp that will definitely be stale (1 second)
            _updatedAt = 1;
        } else {
