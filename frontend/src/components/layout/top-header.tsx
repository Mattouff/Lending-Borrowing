...
import { ConnectWalletButton } from '@/components/simplekit/simplekit';
...
export function TopHeader() {
  ...
  // Exemple de mapping du chainId en nom de réseau
  const networkName =
    chainId === 1
      ? 'Mainnet'
      : chainId === 11155111
    ...
    <Card className="fixed top-0 left-0 z-50 shadow-sm rounded-md bg-sidebar text-sidebar-foreground mt-3 ml-3 w-[calc(100%-1.5rem)]">
      <CardContent className="flex items-center justify-between h-[var(--header-height)]">
        ...
        <div className="flex gap-3 items-center">
          {networkName && address ? (
            <p className="text-sm pr-2">{networkName}</p>
          ) : (
            ''
          )}
          <ConnectWalletButton />
          <Link to={'https://github.com/Mattouff/Lending-Borrowing'}>
            <svg
              stroke="currentColor"
              fill="currentColor"
              stroke-width="0"
